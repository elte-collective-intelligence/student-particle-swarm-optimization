...
def plot_convergence_curves(
    ...
    fig.suptitle("Convergence Curves by Topology", fontsize=_FONT_TITLE + 1,
    ...
    for col, pol in enumerate(policies):
        ...
        for res in pol_res:
            ...
            color = cmap[topo]
            ax.plot(steps, mean_c, color=color, linewidth=2,
...
def plot_diversity_over_time(
    ...
    fig, axes = plt.subplots(nrows, ncols,
                             ...
                             squeeze=False)
    fig.suptitle("Diversity Metrics Over Time", fontsize=_FONT_TITLE + 1,
                 fontweight="bold", y=1.02)
    ...
    for row, met in enumerate(metrics):
        ...
        for col, pol in enumerate(policies):
            ...
            for topo in topologies:
                ...
                ax.plot(steps, curve, color=cmap[topo], linewidth=2,
                        linestyle=_POLICY_LINESTYLE.get(pol, "-"),
                        label=topo, zorder=3)

            title = f"{pol} — {label}" if row == 0 else label
            _apply_style(ax, xlabel="Step", ylabel=label, title=title if row == 0 else "")
            if row == 0:
                ax.legend(fontsize=_FONT_LEGEND, framealpha=0.85)
...
def plot_fitness_distributions(
    ...
    ncols = len(policies)
    fig, axes = plt.subplots(1, ncols, figsize=(max(8, 2.5 * len(topologies)) * ncols, 6),
                             squeeze=False)
    fig.suptitle("Final Fitness Score Distributions", fontsize=_FONT_TITLE + 1,
                 fontweight="bold", y=1.02)
    ...
    for col, pol in enumerate(policies):
        ...
        for res in pol_res:
            ...
            scores = [
                ...
                and not np.isnan(ep["final_score"])
            ]
            if scores:
        ...
        for i, (topo, scores) in enumerate(zip(topologies, violin_data)):
            ...
            ax.scatter(
                positions[i] + jitter, scores,
                color=cmap[topo], s=22, alpha=0.7,
                edgecolors="white", linewidths=0.5, zorder=4,
            )

        ax.set_xticks(positions)
        ax.set_xticklabels(topologies, rotation=15, ha="right", fontsize=_FONT_TICK)
        _apply_style(ax, xlabel="Topology", ylabel="Final best score",
                     title=f"Policy: {pol}")
...
def plot_summary_heatmap(
    ...
    for pol in policies:
        ...
        # --- Figure ---
        fig, ax = plt.subplots(figsize=(max(10, 1.4 * len(_metric_cfg)),
                                        max(4,  0.9 * len(topo_order))))
        ...
        ax.set_title(f"Topology × Metric Summary Heatmap — {pol}",
                     fontsize=_FONT_TITLE, fontweight="bold", pad=10)
        ...
        for row_i in range(len(topo_order)):
            for col_j in range(len(_metric_cfg)):
                ...
                ax.text(col_j, row_i, txt, ha="center", va="center",
                        fontsize=_FONT_TICK - 1, color=txt_color, fontweight="bold")
...
def plot_radar_chart(
    ...
    ax.set_title(f"Topology Radar Chart — {policy}",
                 fontsize=_FONT_TITLE, fontweight="bold", pad=20)
    ...
    ax.legend(loc="upper right", bbox_to_anchor=(1.3, 1.1),
              fontsize=_FONT_LEGEND, framealpha=0.85)
