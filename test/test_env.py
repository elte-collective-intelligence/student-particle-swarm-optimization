...
from envs.dynamic_functions import DynamicRastrigin  # noqa: E402
from envs.dynamic_functions import DynamicEggHolder  # noqa: E402
from utils import LandscapeWrapper  # noqa: E402
from utils import PSOObservationWrapper  # noqa: E402
from utils import PSOActionExtractor  # noqa: E402
...
class TestPSOBasicFunctionality:
    ...
    def test_step(self, basic_env):
        ...
        action = TensorDict(
            {
                "inertia": torch.ones(
                    (basic_env.batch_size[0], 5, 2), device=basic_env.device
                )
                * 0.5,
                "cognitive": torch.ones(
                    (basic_env.batch_size[0], 5, 2), device=basic_env.device
                )
                * 0.5,
                "social": torch.ones(
...
class TestNeighborhoodMechanics:
    ...
    def test_neighborhood_calculation(self, neighborhood_env):
        ...
        neighborhood_env.positions = torch.tensor(
            ...
            device=neighborhood_env.device,
        ).repeat(32, 1, 1)

        neighborhood_env.velocities = torch.zeros_like(neighborhood_env.positions)

        neighborhood_env.delta = 1.5
        avg_pos, avg_vel = get_neighborhood_avg(
            neighborhood_env.positions,
            neighborhood_env.velocities,
            neighborhood_env.delta,
        )

        # For agent at (0,0), neighbors are at (1,0), (0,1), (1,1)
        expected_avg_pos = (
            torch.tensor(
                [[1.0 + 0.0 + 1.0, 0.0 + 1.0 + 1.0]], device=neighborhood_env.device
            ).mean(dim=0)
            / 4
        )  # Includes self
    ...
    def test_delta_effect(self, neighborhood_env):
        ...
        """
        # Set positions
        neighborhood_env.positions = torch.randn(
            neighborhood_env.batch_size[0],
            neighborhood_env.num_agents,
            2,
            device=neighborhood_env.device,
        )
        neighborhood_env.velocities = torch.randn(
            neighborhood_env.batch_size[0],
            neighborhood_env.num_agents,
            2,
            device=neighborhood_env.device,
        ...
        small_avg_pos, _ = get_neighborhood_avg(
            neighborhood_env.positions,
            neighborhood_env.velocities,
            neighborhood_env.delta,
        )
        ...
        large_avg_pos, _ = get_neighborhood_avg(
            neighborhood_env.positions,
            neighborhood_env.velocities,
            neighborhood_env.delta,
        )
...
class TestRewardMechanics:
    ...
    def test_reset_consistency(self, reward_env):
        ...
        assert obs["personal_best_pos"].shape == (
            reward_env.batch_size[0],
            reward_env.num_agents,
            2,
        )
        ...
        assert obs["avg_pos"].shape == (
            reward_env.batch_size[0],
            reward_env.num_agents,
            2,
        )
        assert obs["avg_vel"].shape == (
            reward_env.batch_size[0],
            reward_env.num_agents,
            2,
        )
    ...
    def test_step_changes_state(self, reward_env):
        ...
        action = TensorDict(
            {
                "inertia": torch.ones(
                    (reward_env.batch_size[0], reward_env.num_agents, 2),
                    device=reward_env.device,
                )
                * 0.5,
                "cognitive": torch.ones(
...
class TestUtilsAndWrappers:
    ...
    def test_landscape_wrapper_call_and_dim(self):
        ...
        wrapper = LandscapeWrapper(sum_squares, dim=3)
    ...
    def test_pso_action_extractor_forward(self):
        """
        Test checks if PSOActionExtractor correctly extracts and transforms
        inertia, cognitive, and social components from the action tensor.

        The extractor transforms raw network outputs to proper PSO ranges:
        - inertia: 0.7 + 0.2 * raw (centered at 0.7)
        - cognitive: 1.5 + 0.5 * raw (centered at 1.5)
        - social: 1.5 + 0.5 * raw (centered at 1.5)
        """
        dim = 2
        extractor = PSOActionExtractor(dim, transform_actions=True)

        # Input: (loc, scale) tensors with 3*dim elements each
        # For dim=2: inertia (2), cognitive (2), social (2) = 6 elements
        a0 = torch.tensor([[1.0, 2.0, 1.0, 2.0, 1.0, 2.0]])  # loc
        ...
        # Check shapes
        assert inertia_loc.shape == (1, dim)
        assert cognitive_loc.shape == (1, dim)
        assert social_loc.shape == (1, dim)

        # Check transformed values
        # inertia: 0.7 + 0.2 * [1, 2] = [0.9, 1.1]
        expected_inertia = torch.tensor([[0.9, 1.1]])
        assert torch.allclose(inertia_loc, expected_inertia)

        # cognitive: 1.5 + 0.5 * [1, 2] = [2.0, 2.5]
        expected_cognitive = torch.tensor([[2.0, 2.5]])
        assert torch.allclose(cognitive_loc, expected_cognitive)

        # social: 1.5 + 0.5 * [1, 2] = [2.0, 2.5]
        expected_social = torch.tensor([[2.0, 2.5]])
        assert torch.allclose(social_loc, expected_social)

    def test_pso_action_extractor_untransformed(self):
        """Test extractor with transform_actions=False returns raw values."""
        dim = 2
        extractor = PSOActionExtractor(dim, transform_actions=False)
        ...
        assert torch.allclose(soc_loc, torch.tensor([[5.0, 6.0]]))
